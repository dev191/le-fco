	...
	public class EditBuilding : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if (!Page.IsPostBack)
			{
				...
				BindProvince();							
				ImpostaProvinciaDefault("RM","ROMA");//Carica Anche i Comuni				
				BindUsi();
				BindDitte();
				BindLocalizzazione();
...
//			}
		...
		private void DataGridPiani_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			S_Controls.S_TextBox volumemcadd;
			Classi.ClassiAnagrafiche.Buildings _Buildings = new TheSite.Classi.ClassiAnagrafiche.Buildings();
			int i_Result=0;
			...
			switch(((ImageButton)e.CommandSource).CommandName)
			{
				case "Insert":
					...
					volumemcadd=((S_Controls.S_TextBox) e.Item.FindControl("volumemcadd"));

					if (volumemcadd.Text=="")
						volumemcadd.Text="0";

					volumemcadd.Attributes.Add("onKeyPress","SoloNumeriVirgola();");

					volumemcadd.Text = Convert.ToString(Convert.ToDecimal(volumemcadd.Text));

					string idPiano=cmbPianiAdd.SelectedValue;
					
					if (cmbPianiAdd.SelectedValue!= string.Empty)
						i_Result = classeEdifici.ExecutePianiBuilding("insert", Convert.ToInt64(itemId),Convert.ToInt64(idPiano), Convert.ToDecimal(pianoMqNettiAdd.Text), Convert.ToDecimal(volumemcadd.Text));
					else
					...
					{
						Console.WriteLine(ex.Message);  
						string s_Err = "Errore: Inserimento del piano non riuscito";
						PanelMess.ShowError(s_Err, true);
					}
					finally
					{
		...
		private void DataGridPiani_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			{	
				...
				if(e.Item.Controls[5].Controls[2] is S_Controls.S_TextBox)
					((S_Controls.S_TextBox) e.Item.Controls[5].Controls[1]).Enabled =true;

				if(e.Item.Controls[6].Controls[1] is S_Controls.S_TextBox)
					((S_Controls.S_TextBox) e.Item.Controls[6].Controls[1]).Enabled =true;	
				//btnsSalva.Enabled=false;
			...
			{	
				if(e.Item.Controls[5].Controls[1] is S_Controls.S_TextBox)
				{ 
					((S_Controls.S_TextBox) e.Item.Controls[5].Controls[1]).Enabled =true;
					if (((S_Controls.S_TextBox) e.Item.Controls[5].Controls[1]).Text =="")
						((S_Controls.S_TextBox) e.Item.Controls[5].Controls[1]).Text ="0";
				}
				if(e.Item.Controls[6].Controls[1] is S_Controls.S_TextBox)
				{
					((S_Controls.S_TextBox) e.Item.Controls[6].Controls[1]).Enabled =true;
		...
		private void DataGridPiani_UpdateCommand_1(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			S_Controls.S_TextBox volumemcMod;
			Classi.ClassiAnagrafiche.Buildings _Buildings = new TheSite.Classi.ClassiAnagrafiche.Buildings();
			...
			try
			{
				...
				volumemcMod = ((S_Controls.S_TextBox) e.Item.FindControl("volumemcMod"));

				if (volumemcMod.Text=="")
					volumemcMod.Text="0";

				volumemcMod.Attributes.Add("onKeyPress","SoloNumeriVirgola();");
				
				volumemcMod.Text = Convert.ToString(Convert.ToDecimal(volumemcMod.Text));

				string idPiano=cmbPianiMod.SelectedValue;
				i_Result = _Buildings.ExecutePianiBuilding("update", Convert.ToInt64(itemId), Convert.ToInt64(idPiano),  Convert.ToDecimal(pianoMqNettiMod.Text), Convert.ToDecimal(volumemcMod.Text));
