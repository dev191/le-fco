...
//				this.cmbsGruppo.DataValueField = "richiedenti_tipo_id";
		...
		private S_ControlsCollection GetData()
		{
			...
			S_Controls.Collections.S_Object s_p_sga = new S_Controls.Collections.S_Object();
			s_p_sga.ParameterName = "p_sga";
			s_p_sga.DbType = ApplicationDataLayer.DBType.CustomDBType.VarChar;
			s_p_sga.Direction = ParameterDirection.Input;
			s_p_sga.Index = _SCollection.Count;
			s_p_sga.Size=50;
			s_p_sga.Value = txtSGA.Text;		
			_SCollection.Add(s_p_sga);
...
//			p_prog.Value = RicercaModulo1.idProg.Value;  			
		...
		private void DataGridRicerca_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			{ 
				...
				{
					string destDir =  "../Doc_DB/" + dr["bl_id"].ToString() +  "/" + dr["WR_ID"].ToString() + "/PREV/" +dr["pdfpreventivo"].ToString().Replace("'","`");
					_linkPre.HRef=destDir; 
					_linkPre.Target ="_blank";
//					_linkPre.HRef="javascript:openpdf('" + dr["WR_ID"].ToString() + "',  + 'PREV','" + dr["pdfpreventivo"].ToString().Replace("'","`")  + "');"; 
					...
					_imagepre.Attributes.Add("title","Preventivo");
...
//					LkPrev.NavigateUrl =destDir;
				...
				{
					string destDir =  "../Doc_DB/" + dr["bl_id"].ToString() +  "/" + dr["WR_ID"].ToString() + "/CONS/" +dr["pdfconsuntivo"].ToString().Replace("'","`");
					_linkCon.HRef=destDir;
					_linkCon.Target ="_blank";
