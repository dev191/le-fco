...
//			}
		...
		{
			Classi.CostiOperativi.Richiesta  _Richiest = new TheSite.Classi.CostiOperativi.Richiesta();						
			...
			if(Count==false)
			{
				...
				_MyDs = _Richiest.GetSfogliaRDL_r1(_SCollection).Copy();	
			}else
				_MyDs = _Richiest.GetSfogliaRDLCount_r1(_SCollection).Copy();	
		...
		private void RicercaManRichiesta(bool reset)
		{
			...
			Classi.CostiOperativi.Richiesta  _Richiesta = new TheSite.Classi.CostiOperativi.Richiesta();						
			S_ControlsCollection _SCollection = new S_ControlsCollection();			
			...
			DataSet _MyDs = _Richiesta.GetSfogliaRDL_r2(_SCollection).Copy();		
			...
			if (reset==true)
			{
				...
				_SCollection.RemoveAt(_SCollection.Count -1);
				_SCollection.RemoveAt(_SCollection.Count -1);
				int _totalRecords = _Richiesta.GetSfogliaRDLCount_r2(_SCollection);
