...
//			}
			...
			if(this.IsCompleta==true)//completamento
			{
				...
				if (_Dr["lettura_contatore"] != DBNull.Value) 
				{
				  LABEL1.Text="Ultima lettura contatore (h): "+_Dr["lettura_contatore"].ToString();
...
//			_SColl.Add(p);
		...
		private void insdaticontantore()
		{	
			
			S_ControlsCollection _SColl1 =GetSgaParametercontatore();
			
			int result= _ClManCorrettiva.insdaticontantore(_SColl1); 
			
		}

		private S_ControlsCollection GetSgaParametercontatore()
		{
			S_ControlsCollection _SColl1 = new S_ControlsCollection();
			...
			p = new S_Object();
			p.ParameterName = "p_lettura";
			p.DbType = CustomDBType.VarChar;
			p.Direction = ParameterDirection.Input;
			p.Index = _SColl1.Count;
			if (txtlettura.Text=="") 
			{txtlettura.Text="0";}
			p.Value = txtlettura.Text;
			_SColl1.Add(p);
...
//			Response.Redirect("SfogliaRdlPaging6.aspx?FunId=" + ViewState["FunId"] + varapp);
			Server.Transfer("SfogliaRdlPaging6.aspx");  
//			string varapp="";
//			if(Request["VarApp"]!=null)
//				varapp="&VarApp=" + Request["VarApp"];
//			
...
//		}
		...
		{
			SaveCompletamento();
			if (txtlettura.Text!=string.Empty)
			insdaticontantore();
			LoadDati();
			if (Datapanel5.Visible==true)
			{
