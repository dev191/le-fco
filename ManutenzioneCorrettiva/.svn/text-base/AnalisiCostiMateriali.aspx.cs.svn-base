...
namespace TheSite.ManutenzioneCorrettiva
{
	...
	public class AnalisiCostiMateriali : System.Web.UI.Page
	{
		...
		{    
			...
			this.ExpPdf.Click += new System.EventHandler(this.ExpPdf_Click);
		...
		private void Ricerca()
		{
			...
			//ArrayList arr_num_interruzioni=new ArrayList();
			string curCat; 
			string prevCat=string.Empty; 
			int i = 0; 
			int QtaIn=0;
			int QtaOut=0;
			dt.Columns.Add("Contatore");
			if (Ds.Tables[0].Rows.Count > 0) 
			{
				while (i <= dt.Rows.Count - 1) 
				{ 					
					dt.Rows[i]["Contatore"]=i;
				...
				if(DataGridRicerca.CurrentPageIndex==0)
				{					
					lastindex=(startIndex + DataGridRicerca.PageSize);		
					dv.RowFilter="Contatore>="+startIndex +" and Contatore <"+lastindex;
				}
				else
				{
					startIndex=DataGridRicerca.CurrentPageIndex * DataGridRicerca.PageSize;
					lastindex=(startIndex + DataGridRicerca.PageSize); 
					dv.RowFilter="Contatore>="+startIndex +" and Contatore <"+lastindex;
				}
				
				DataGridRicerca.DataSource=dv;
				DataGridRicerca.VirtualItemCount = Ds.Tables[0].Rows.Count;
		...
		private void DataGridRicerca_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType== ListItemType.Footer)
			{
				...
				System.Web.UI.WebControls.ImageButton _img2 = (System.Web.UI.WebControls.ImageButton) (e.Item.FindControl("imbInsert"));								
				if(_img2!=null)
				{
					_img2.Attributes.Add("onclick", "return checkMatId('" + ((TextBox)(e.Item.FindControl("IdMateriale"))).ClientID + "');");	
			...
			if(e.Item.ItemType == ListItemType.Item || e.Item.ItemType== ListItemType.AlternatingItem)
			{
				...
				//				
				if (Convert.ToInt32(Dv["quantita"].ToString())>0)
				{
					e.Item.Cells[3].Text="+";
		...
		private void DataGridRicerca_UpdateCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			txtCalcolaTotale  = (TextBox) e.Item.FindControl("txttotaleEdit");
			DataRowView Dv= (DataRowView)e.Item.DataItem;
			IdMateriale  = (TextBox) e.Item.FindControl("IdMaterialeEd");
			string appo=(cmbMateriali.SelectedValue).ToString().Split(';')[0];
			int idMateriale       = Convert.ToInt32(appo);
		...
		private void DataGridRicerca_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			DropDownList cmbMateriali;
			TextBox txtPrezzoUnitario, txtQuantita, txtCalcolaTotale,IdMateriale;
			if (e.CommandName=="Insert" || e.CommandName=="Delete")
			{
		...
		private void ExpPdf_Click(object sender, System.EventArgs e)
		{
			Ricerca();
		}
