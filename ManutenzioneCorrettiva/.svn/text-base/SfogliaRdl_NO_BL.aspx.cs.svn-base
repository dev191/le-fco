...
namespace TheSite.ManutenzioneCorrettiva
{
	...
	/// </summary>
	public class SfogliaRdl_NO_BL :  System.Web.UI.Page
	{
		#region variabili
		...
		public  string codstatoRdl = string.Empty;	
		public  string codblid = string.Empty;
		#endregion
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			string _mypage="./ManutenzioneCorrettiva/SfogliaRdl_NO_BL.aspx";			
...
//			RicercaModulo1.DelegateCodiceEdificio1 +=new  WebControls.DelegateCodiceEdificio(this.BindServizio);
			codblid=Request.QueryString["bl_id"].ToString();
			codstatoRdl=Request.QueryString["codstato"].ToString();
			...
			{	
				...
				BindServizio(codblid);
				BindGruppo();
				BindUrgenza("-1");
				BindStatus(codstatoRdl);	
				...
				RicercaManRichiesta(true);//manutenzione a Richiesta
		...
		private void BindStatus(string statordl)
		{
			...
			if (Ds.Tables[0].Rows.Count > 0)
			{
				...
				this.cmbsStatus.SelectedValue=statordl;
			}
			else
			{
		...
		private S_ControlsCollection GetData()
		{
			...
			s_p_Bl_Id.Size =50;
			s_p_Bl_Id.Index = 0;
			s_p_Bl_Id.Value = codblid;//RicercaModulo1.TxtCodice.Text;
		...
		{
			Response.Redirect("SfogliaRdlPaging5.aspx?FunID=" + ViewState["FunId"]);
		}
		...
		private void RicercaManRichiesta(bool reset)
		{
			...
			if (reset==true)
			{
				...
				int _totalRecords = _Richiesta.GetCount1(_SCollection);
			...
			DataSet _MyDs = _Richiesta.GetSfogliaRDL1(_SCollection).Copy();		
		...
		public DataSet GetWordExcel()
		{
			...
			return  _Richiesta.GetSfogliaRDLExcel3(_SCollection).Copy();		
		}
		...
		private void DataGridRicerca2_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
		{
			...
			{				
				_myColl.AddControl(this.Page.Controls,ParentType.Page );
				string s_url = e.CommandArgument.ToString();		
				string IdStato = e.Item.Cells[16].Text.Trim();	
				string appo2 ="CompletaRdl.aspx?wr_id=" +(e.Item.Cells[0].Text) + varapp;
				string appo="CompletaRdl.aspx?wr_id=" +(e.Item.Cells[0].Text) + varapp;
				switch (IdStato)
				{
