...
namespace TheSite.ManutenzioneCorrettiva
{
	...
	/// </summary>
	public class CreazioneSGA3 : System.Web.UI.Page
	{
		...
		protected System.Web.UI.WebControls.DropDownList cmbsrichiedente;
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			string _mypage="./ManutenzioneCorrettiva/CreazioneSGA3.aspx";			
			Classi.SiteModule _SiteModule = new TheSite.Classi.SiteModule(_mypage);
			// ***********************  MODIFICA PER I PERMESSI SULLA PAGINA CORRENTE **********************

			FunId = _SiteModule.ModuleId;
			
			//HelpLink = "../HelpApplication/Default.aspx?page=ManutenzioneCorrettiva/CreazioneSGA3.aspx";
...
//				BindApparecchiatura("SL");
		...
		// freeze apparato 3 //
		/// <summary>
		/// Popola i dettagli dell'apparato
		/// </summary>
		private void dettaglioApparato()
		{
			...
			BindServizio(RicercaModulo1.TxtCodice.Text);
			BindApparecchiatura(""+servizi_id);
		}
		// freeze apparato 3 //
		...
		private void LoadRichiedente()
		{
			_ClManCorrettiva=new TheSite.Classi.ManCorrettiva.ClManCorrettiva();

			DataSet _MyDs =  _ClManCorrettiva.GetRichiedente();

			if (_MyDs.Tables[0].Rows.Count>0)
			{
				cmbsrichiedente.DataSource = _MyDs.Tables[0];
				this.cmbsrichiedente.DataTextField = "descrizione";
				this.cmbsrichiedente.DataValueField = "id";
				this.cmbsrichiedente.DataBind();
				//CmbProgetto.SelectedValue="1";
		...
		private void btnsSalva_Click(object sender, System.EventArgs e)
		{
			...
			if (_Richiesta.IsValidBlId(this.RicercaModulo1.BlId))
			{
				...
				{	
					...
					li= _mailsend.GetDestinatari_rv(Int32.Parse(RicercaModulo1._txthidbl.Value),Int32.Parse(cmbsServizio.SelectedValue),DocType.SGA, Int32.Parse(cmbsTipoManutenzione.SelectedValue));
						
					//string[] indirizzi= txtsMittente.Text.Split(Convert.ToChar(";"));   
					Classi.ClassMail _mail=new TheSite.Classi.ClassMail();
					try
					{
						foreach(string indirizzo in li)
						{
							...
							//								_messaggio.Subject="Avviso di creazione rdl nr: "+i_Wr_Id.ToString(); 
							_messaggio.To =indirizzo.Trim();
							_messaggio.Cc=System.Configuration.ConfigurationSettings.AppSettings["CCSGA"];
							...
							_mail.Decription="Apparato : " + CodiceApparecchiature1.CodiceApparecchiatura+" Descrizione Problema-Note: " + txtsProblemaBreve.Text+" "+ txtsProblema.Text;
							_mail.Urgenza=cmbsUrgenza.SelectedItem.ToString();
							_mail.Tipomanutenzione=cmbsTipoManutenzione.SelectedItem.ToString();
							_mail.Servizio=cmbsServizio.SelectedItem.ToString();
							_mail.Richiedente=cmbsrichiedente.SelectedItem.ToString();
							...
							DataSet _MyDs;
							_MyDs = _ClManCorrettiva.BlInfo(_SColl);
							if( _MyDs != null && _MyDs.Tables[0]!=null && _MyDs.Tables[0].Rows[0]!=null && _MyDs.Tables[0].Rows[0].ItemArray[0]!=null)
							{
								bl_info = _MyDs.Tables[0].Rows[0].ItemArray[0].ToString();
								_mail.CodiceEdificio = bl_info;
							}
							...
							_MyDs1= _mailsend.GetaltreEmail_RV(Int32.Parse(RicercaModulo1._txthidbl.Value),Int32.Parse(cmbsServizio.SelectedValue),DocType.SGA,
								indirizzo,Int32.Parse(cmbsTipoManutenzione.SelectedValue) );
							altremail= _MyDs1.Tables[0].Rows[0].ItemArray[0].ToString();
							_messaggio.Subject="Creazione rdl nr: "+i_Wr_Id.ToString()+" Edificio: "+ bl_info; 
								
							_mail.Altreemail=altremail;
							_messaggio.To=indirizzo.Trim()+";"+altremail;
							int num=_mail.SendMail_log(Classi.ClassMail.TipoMail.MailCreazioneRichiesta); 
						}
						}
							catch (Exception ex)
						{
					...
					{
						string[] indirizzi= txtsMittente.Text.Split(Convert.ToChar(";"));   
						Classi.ClassMail _mail_=new TheSite.Classi.ClassMail();
						try
						{
							foreach(string indirizzo in indirizzi)
							{
								...
								_messaggio.BodyFormat =MailFormat.Html; 
								_mail_.Messaggio =_messaggio;
								_mail_.Idrichiesta =i_Wr_Id.ToString();
								_mail_.Name =indirizzo.Trim();
								_mail_.Decription=txtsProblema.Text;
								_mail_.CodiceEdificio =RicercaModulo1.BlId + " " + RicercaModulo1.Nome
									+ " Comune: " + RicercaModulo1.Comune;
								//_mail.Surname =indirizzo;
								_mail_.SendMail(Classi.ClassMail.TipoMail.MailCreazioneRichiesta); 
							} 
		...
		private int NuovaRichiesta()
		{
			...
			//-------
			S_Controls.Collections.S_Object s_p_id_em = new S_Object();
			s_p_id_em.ParameterName = "p_id_em";
			s_p_id_em.DbType = ApplicationDataLayer.DBType.CustomDBType.Integer;
			s_p_id_em.Direction = ParameterDirection.Input;
			s_p_id_em.Size = 10;
			s_p_id_em.Index = _SColl.Count;
			s_p_id_em.Value=Convert.ToInt32(cmbsrichiedente.SelectedValue);	
			_SColl.Add(s_p_id_em);
			...
			s_p_Eqstd_Id.Value=(cmbsApparecchiatura.SelectedValue ==string.Empty)? 0: Convert.ToInt32(cmbsApparecchiatura.SelectedValue);
...
//			_SColl.Add(s_p_Description);
			...
			{
				if (ItemId != 0)
					i_WrId = _Richiesta.ModificaRDL_Rev1(_SColl,ItemId);
				else
					i_WrId = _Richiesta.Crea_Rev1(_SColl);
			}
			catch (Exception ex)
			{
		...
		{
			Response.Redirect("CreazioneSGA3.aspx?FunId=" + ViewState["FunId"]);
		...
		private void LoadDatiCreazione(int ItemId)
		{
			...
			{	
				...
				if (_Dr["em_id"] != DBNull.Value)
				{
					LoadRichiedente();
					cmbsrichiedente.SelectedValue=_Dr["em_id"].ToString();
				}
