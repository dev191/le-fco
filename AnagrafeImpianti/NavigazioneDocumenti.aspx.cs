...
//			this.S_btRicerca.Attributes.Add("onclick", sbValid.ToString());
		...
		private void LoadEstensione()
		{
			...
			if (_Ds.Tables[0].Rows.Count > 0)
			{
				this.S_CmbTipologia.DataSource = Classi.GestoreDropDownList.ItemBlankDataSource(
					_Ds.Tables[0], "valTipo", "documento_id", "- Selezionare una Estensione File -", "");
		...
		private void LoadTipologia()
		{
			Classi.AnagrafeImpianti.AnagrafeServizi   _AnagrafeServizi = new Classi.AnagrafeImpianti.AnagrafeServizi(Context.User.Identity.Name);
			DataSet _Ds=_AnagrafeServizi.GetTipologiaDOC();
			if (_Ds.Tables[0].Rows.Count > 0)
			{
				this.S_CbTipologia.DataSource = Classi.GestoreDropDownList.ItemBlankDataSource(
					_Ds.Tables[0], "descrizione", "id_tipologia", "- Selezionare una Tipologia -", "");
				this.S_CbTipologia.DataTextField = "descrizione";
				this.S_CbTipologia.DataValueField = "id_tipologia";
...
//			a_appoggio1=a_appoggio[0].ToString().Split(Convert.ToChar("&"));
			...
			else
			{
				...
				s_p_filname.Index = CollezioneControlli.Count;
