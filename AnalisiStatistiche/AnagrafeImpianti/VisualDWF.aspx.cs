...
namespace TheSite.AnagrafeImpianti
{
	...
	public class VisualDWF : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			//string ctr=Context.Items["var_doc"].ToString();
			if(!IsPostBack)
			{
				...
				if (Context.Items["var_afm_dwgs_dwg_name"]!=null)
				{
					...
					//this._descrizione=(string)Context.Items["var_afm_dwgs_dwg_title"];
					
					Execute();
				}
				if (Context.Items["var_doc"]!=null )
				{
					string[] a_appoggio =Context.Items["var_doc"].ToString().Split(Convert.ToChar("?"));
					this.filname=a_appoggio[1];
					this._descrizione=a_appoggio[0];
					this.vvf =(Context.Items["var_vvf"]==null)?string.Empty:(string)Context.Items["var_vvf"];
					this.ispesl =(Context.Items["var_ispesl"]==null)?string.Empty:(string)Context.Items["var_ispesl"];
				...
				if (Request.QueryString["var_doc"]!=null)
				{
					string[] a_appoggio =Request.QueryString["var_doc"].ToString().Split(Convert.ToChar("?"));
					this.filname=a_appoggio[1];
					this._descrizione=a_appoggio[0];
					this.vvf =(Request.QueryString["var_vvf"]==null)?string.Empty:(string)Request.QueryString["var_vvf"];
					this.ispesl =(Request.QueryString["var_ispesl"]==null)?string.Empty:(string)Request.QueryString["var_ispesl"];
		...
		private void Execute()
		{
			...
			S_Controls.Collections.S_Object s_p_filename = new S_Controls.Collections.S_Object();
			s_p_filename.ParameterName = "p_filename";
			s_p_filename.DbType = CustomDBType.VarChar;
			s_p_filename.Direction = ParameterDirection.Input;
			s_p_filename.Index = 0;
			s_p_filename.Value = this.filname;			
			s_p_filename.Size = 100;
			CollezioneControlli.Add(s_p_filename);
			...
			s_p_descrizione.Value = this._descrizione;			
		...
		private void GetFile(string PathRoot,string PathChild)
		{
			...
			if (PathRoot=="EQAllegati")
			{PathFileName="../EQAllegati/"+ this.filname;}
			else
